  ...
  return (
    <details className="relative">
      <summary aria-label="Account menu" className="flex cursor-pointer list-none items-center gap-2 rounded-full border border-slate-200 bg-slate-50 p-1.5 text-slate-600 hover:border-violet-200 hover:bg-violet-50 hover:text-violet-700 sm:pr-3">
        ...
        <span className="hidden max-w-40 truncate text-xs font-bold sm:block">{email ?? (configured ? "Account" : "Profile")}</span>
      </summary>
      <div className="absolute right-0 top-12 z-50 w-64 rounded-2xl border border-slate-200 bg-white p-2 shadow-xl">
        <div className="px-3 py-2"><div className="text-xs font-bold uppercase tracking-wider text-slate-400">{configured ? "Signed in as" : "Browser profile"}</div><div className="mt-1 truncate text-sm font-semibold text-slate-800">{email ?? (configured ? "Loading account…" : "Saved on this device")}</div></div>
